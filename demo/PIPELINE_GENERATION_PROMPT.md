# Pipeline Image Generation Prompt (V5 — Corrected)
...
**Fix:** All titles must use V5 stage numbering (0–7). All references to "V4" must be "V5".

### Error 4: Flat-Field Shows Fixed σ=45

**Problem:** `method_flat_field.png` title says "σ=45" — this was the V4 fixed value.

**Fix:** Must show "σ = 0.07·D" (V5 adaptive sigma). The flat-field formula is: `I' = I − GaussianBlur(I, σ=0.07·D) + μ_FOV`, where D is the FOV diameter.

### Error 5: Baseline vs V5 Images Show Same Processing

**Problem:** In `before_after_pipeline.png` and `baseline_vs_pipeline.png`, the V5 side still has cropping/rotation artifacts, reducing the visual contrast with baseline.
...
- **V5 Pipeline** = Stage 0 → Stage 2 (margin_pct=0) → Stage 4 → Stage 5. Circle inscribed in square, uniform illumination, enhanced contrast.

The visual distinction should be immediately obvious: baseline fills the square (distorted ellipse) vs V5 preserves the circle (inscribed, with black corners).

---

## Source Images

**Обязательное требование:** ВСЕ pipeline-изображения должны быть получены обработкой реальных снимков из директории `E:\dissertation-project\demo\public\fundus-examples\dr04\`. Никакие синтетические, сгенерированные или placeholder-изображения не допускаются. Каждый файл в `public/pipeline/` — это результат прогона реального фундус-снимка через соответствующие стадии V5 пайплайна.
...
# V5 pipeline (NO Stage 1 rotation)
...
- Baseline uses `baseline_processing()` (stretch-resize), NOT V5 stages
...
- **Status:** EXISTS — needs regeneration
- **Content:** 3×3 grid showing right_eye.jpeg through all V5 stages
...
- **Title:** "V5 Pipeline Stages — Patient 43199 (DR4, Proliferative DR)"
...
- **Status:** EXISTS — needs regeneration
- **Content:** 2×3 grid. Top row: Right eye (OD) raw → Cropped 512×512 → Full V5. Bottom row: Left eye (OS) raw → Flipped + Cropped → Full V5.
- **Title:** "Bilateral Pair — Canonical Flip + Full V5 Pipeline / Patient 43199 (DR4)"
...
- **Status:** EXISTS — needs regeneration
- **Content:** Side-by-side: Baseline (3ch) vs Full V5 Pipeline (4ch)
- **Left panel:** Baseline — stretch-resize to 512×512 (fills entire square, distorted elliptical FOV). Badge: "3ch RGB" (gray).
- **Right panel:** V5 — isotropic resize with flat-field + CLAHE (circular FOV inscribed in square, black corners). Badge: "4ch RGBM" (teal).
- **Title:** "Baseline Processing vs Full V5 Pipeline"
- **Caption:** "V5 isotropic resize preserves circular FOV geometry; flat-field corrects illumination; CLAHE enhances vessel contrast"
- **Fix needed:** Regenerate with corrected V5 processing. The visual contrast between baseline (distorted, filling square) and V5 (circle inscribed, enhanced) must be immediately obvious.

#### 11. `baseline_vs_pipeline.png`
- **Status:** EXISTS — needs regeneration
- **Content:** 2×2 grid comparison
- **Top-left:** Baseline stretch-resize (distorts geometry)
- **Top-right:** V5 isotropic resize (preserves circular FOV)
- **Bottom-left:** Baseline (no enhancement, original illumination)
- **Bottom-right:** V5 flat-field + CLAHE (uniform illumination, enhanced contrast)
- **Title:** "Baseline Stretch-Resize vs V5 Isotropic Resize"
...
- **Status:** EXISTS — needs regeneration
- **Content:** Comparison table: Standard approach vs V5 fundus-specific adaptations for each pipeline stage
- **Title:** "V5 Pipeline — Standard vs. Fundus-Specific Adaptations"
- **Fix needed:** Title says "V4 Pipeline" → change to "V5 Pipeline". Update table content:
...
7. `before_after_pipeline.png` — V5 side has artifacts
8. `baseline_vs_pipeline.png` — V5 side has artifacts

**Must fix titles (V4 → V5 numbering):**
...
15. `methods_comparison_table.png` — "V4" → "V5"
...
- [ ] All titles use V5 stage numbering (0–7)
...
- [ ] V5 images use isotropic resize (circle inscribed, black corners)
...
- [ ] `methods_comparison_table.png` — says "V5 Pipeline" with correct stage numbers
