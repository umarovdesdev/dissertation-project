...
import { IDRID_SAMPLES } from './_idridSamples';
...
// (regenerated by `demo/scripts/prepare_idrid_samples.py`).
//
// IDRiD is monocular (one image per record), so a pick fills a single eye slot
// and carries a `gt` payload (OD/fovea centres + FOV mask in DISPLAY-frame
// pixels). Because the markers are in the displayed image's own frame, they
// overlay by simple scaling — no canonical-flip / rotation / crop transform —
// which is exactly why they land correctly.
// ---------------------------------------------------------------------------
let lastIdridIdx = -1;
function pickRandomIdridSample() {
  if (!IDRID_SAMPLES || IDRID_SAMPLES.length === 0) return null;
  let idx;
  do {
    idx = Math.floor(Math.random() * IDRID_SAMPLES.length);
  } while (IDRID_SAMPLES.length > 1 && idx === lastIdridIdx);
  lastIdridIdx = idx;
  return IDRID_SAMPLES[idx];
}

// Walkthrough fallback (used only when IDRID_SAMPLES is empty).
let lastWalkIdx = -1;
...
export default function Demo() {
  ...
  // Roll a random IDRiD localization sample. IDRiD is monocular and ships
  // ground-truth OD + fovea centres, so we load one image into the RIGHT slot
  // (no canonical flip → display frame == analysis orientation) and attach a
  // `gt` payload that VisionWidget overlays directly. If the IDRiD manifest is
  // empty (prep script not yet run), fall back to a walkthrough bilateral case.
  const loadRandomSample = () => {
    const s = pickRandomIdridSample();
    if (s) {
      const gt = {
        source: 'IDRiD',
        id: s.id,
        grade: s.grade,
        odCenter: s.od_center,       // [x, y] in display-frame pixels
        foveaCenter: s.fovea_center, // [x, y] in display-frame pixels
        odRadius: s.od_radius,
        width: s.width,
        height: s.height,
        maskSrc: process.env.PUBLIC_URL + '/' + s.mask,
      };
      setRightImg({ src: process.env.PUBLIC_URL + '/' + s.image, name: s.id, fromUpload: false, gt });
      setLeftImg(null); // monocular sample — single eye
    ...
    <div>
      ...
      <Sec title={t('demo.uploadSection')}>
        ...
        {/* Per-image "what the model sees" widget (TASK-Demo D.2). Backed by
            /api/visualize (preprocessing-only — no checkpoint needed). IDRiD
            samples carry ground-truth markers (`gt`) and render even offline. */}
        {(liveVisuals || leftImg?.gt || rightImg?.gt) && (leftImg || rightImg) && (
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', marginTop: 12 }}>
            {leftImg && (liveVisuals || leftImg.gt) && (
              <VisionWidget src={leftImg.src} eye="left" name={leftImg.name} enabled={liveVisuals} gt={leftImg.gt} t={t} />
            )}
            {rightImg && (liveVisuals || rightImg.gt) && (
        ...
        <div style={{ marginTop: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            ...
            >
              🎲 {t('demo.random')}
              {IDRID_SAMPLES && IDRID_SAMPLES.length > 0 && (
                <span style={{ marginLeft: 4, opacity: 0.6, fontWeight: 400 }}>
                  IDRiD ({IDRID_SAMPLES.length})
                </span>
              )}
            </button>
          </div>
