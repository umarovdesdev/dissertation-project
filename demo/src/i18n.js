...
const translations = {
  en: {
    ...
    'demo.subtitle': 'Upload up to two fundus images of a single patient (left and right eye). The trained model (Config D) returns a 5-class diabetic retinopathy grade. Confirm or reject the prediction — feedback is added to a relabeling buffer for further training.',
    'demo.disclaimer': 'Research demo only — not for clinical use. Predictions are simulated on the client side using the fixed performance profile of Config D (F1=0.780, AUC=0.865).',
    'demo.uploadSection': '1. Provide images',
    'demo.leftEye': 'Left eye (OS)',
    'demo.rightEye': 'Right eye (OD)',
    ...
    'demo.random': 'Random patient (EyePACS)',
    'demo.randomHint': 'Loads a random EyePACS bilateral pair: {patient_id}_left.jpeg goes into the left slot, _right.jpeg into the right slot. Run `python demo/scripts/prepare_eyepacs_pairs.py` once to populate the pool from E:\\datasets\\eyepacs; until then this button falls back to a random walkthrough case.',
    ...
    'demo.modelVersion': 'Model',
    'demo.modelStatus.checking': 'checking model…',
    'demo.modelStatus.real': '● real model',
    'demo.modelStatus.simulator': '● simulator (backend offline)',
    'demo.modelStatus.useReal': 'use real model',
    'demo.modelStatus.useSimulator': 'use simulator',
    'demo.modelStatus.fallbackToast': 'Inference failed — falling back to the simulator.',
    'demo.modelStatus.footerReal': 'Predictions from the Config D checkpoint (EfficientNet-B3), served live by the backend.',
    'demo.modelStatus.footerSim': 'Simulated Config D output (backend offline).',
    'demo.framing.line1': 'Diabetic Retinopathy Diagnosis Demo — V5 Preprocessing + EfficientNet-B3 (Config D)',
    ...
    'demo.vision.stripCaption': 'V5 stages: original → flip → OD-fovea rotation → FOV crop → flat-field → CLAHE',
    'demo.gradcam.loading': 'computing Grad-CAM…',
    'demo.gradcam.error': 'Grad-CAM unavailable',
    'demo.viz.liveDisclaimer': 'Live Grad-CAM from the Config D checkpoint. Interpretability evidence only — not clinical lesion localization.',
    ...
    'tooltip.fig03': 'Preprocessing improvement delta: ResNet-50 +5.2pp F1, EfficientNet-B3 +5.3pp F1. Both exceed EH-3 threshold (red line).',
    ...
    'tooltip.fig12': 'EH-3 check: both architectures exceed all thresholds. ResNet-50 +5.2/+3.3/+8.0pp, EfficientNet-B3 +5.3/+4.4/+8.0pp.',
    ...
    'tooltip.fig21': 'Statistical tests. ResNet-50: DeLong p=0.006, McNemar p=0.009. EfficientNet-B3: p=0.008, p=0.012. All significant at α=0.05.',
    'tooltip.fig22': 'All 4 configurations A–D. Both architectures benefit: ResNet-50 (B vs A) +5.2pp F1, EfficientNet-B3 (D vs C) +5.3pp F1. Config D achieves highest absolute F1.',
  ...
  kz: {
    ...
    'demo.subtitle': 'Бір пациенттің екі көзіне дейін (сол және оң) көз түбі суреттерін жүктеңіз. Үйретілген модель (Config D) 5-класты DR деңгейін қайтарады. Болжамды растаңыз немесе қабылдамаңыз — кері байланыс қайта оқыту буферіне жазылады.',
    'demo.disclaimer': 'Тек зерттеу демосы — клиникалық қолдануға арналмаған. Болжамдар Config D профилі негізінде клиент жағында үлгіленеді (F1=0.780, AUC=0.865).',
    'demo.uploadSection': '1. Кескіндерді ұсыну',
    'demo.leftEye': 'Сол көз (OS)',
    'demo.rightEye': 'Оң көз (OD)',
    ...
    'demo.random': 'Кездейсоқ пациент (EyePACS)',
    'demo.randomHint': 'EyePACS-тен кездейсоқ пациенттің екі көз түбі суретін жүктейді: {patient_id}_left.jpeg сол слотқа, _right.jpeg оң слотқа. Алдымен бір рет `python demo/scripts/prepare_eyepacs_pairs.py` орындаңыз, ол E:\\datasets\\eyepacs-тен жұптарды дайындайды. Әзірге пул бос болса, батырма walkthrough кейстерінен таңдайды.',
    ...
    'demo.modelVersion': 'Модель',
    'demo.modelStatus.checking': 'модель тексерілуде…',
    'demo.modelStatus.real': '● нақты модель',
    'demo.modelStatus.simulator': '● симулятор (backend офлайн)',
    'demo.modelStatus.useReal': 'нақты модельді қолдану',
    'demo.modelStatus.useSimulator': 'симуляторды қолдану',
    'demo.modelStatus.fallbackToast': 'Инференс сәтсіз — симуляторға ауысу.',
    'demo.modelStatus.footerReal': 'Болжамдар backend арқылы тікелей берілген Config D чекпойнтінен (EfficientNet-B3).',
    'demo.modelStatus.footerSim': 'Симуляцияланған Config D нәтижесі (backend офлайн).',
    'demo.framing.line1': 'Диабеттік ретинопатияны диагностикалау демосы — V5 алдын ала өңдеу + EfficientNet-B3 (Config D)',
    ...
    'demo.vision.stripCaption': 'V5 кезеңдері: түпнұсқа → аудару → OD-фовеа бұру → FOV кесу → flat-field → CLAHE',
    'demo.gradcam.loading': 'Grad-CAM есептелуде…',
    'demo.gradcam.error': 'Grad-CAM қолжетімсіз',
    'demo.viz.liveDisclaimer': 'Config D чекпойнтінен тікелей Grad-CAM. Тек интерпретация дәлелі — клиникалық локализация емес.',
