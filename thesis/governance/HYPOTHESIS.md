...
**Conclusion:** Application of the preprocessing pipeline prior to CNN classification improves diagnostic performance (H-1), is robust to parameter variation (H-2), transfers across datasets (H-4), directs attention to clinically relevant lesions (H-5), generalizes across imaging devices (H-6), and reduces clinical degradation on external datasets (H-7).
