...
**Premise 4 (In-Domain Pretraining Provides Retina-Aware Initialization) [v5.1].** Retinal foundation models (RETFound, Zhou et al., 2023) trained via masked autoencoding on large-scale unlabeled fundus corpora produce initialization weights whose features are aligned with retinal anatomical structures (vasculature, optic disc, macula, lesions). In-domain initialization is expected to improve sample efficiency and clinical generalization relative to natural-image (ImageNet) initialization.
