...
- **Independent variable (composite, v5.1 amended; pretraining source revised in v6.0.0):** The combined factor *(preprocessing × pretraining source)*. The baseline arm and the V5 arm differ jointly along the preprocessing axis (stretch-resize + ImageNet normalize vs. full V5 pipeline) **and** along the pretraining axis (ImageNet vs. ophthalmology-specific self-supervised pretraining). Single-factor isolation along preprocessing alone — the form of H-1 in v5.0 — is no longer obtained and is explicitly relinquished by this amendment.
...
- **V5 arm (H-1 V5 configurations):** Initialization derived from **ophthalmology-specific self-supervised pretraining** of the same CNN backbone (ResNet-50 or EfficientNet-B3) on an unlabeled retinal fundus corpus, using a CNN-compatible domain-adaptive SSL protocol (DINO / BYOL / SimCLR / MoCo family; the specific protocol is selected empirically and recorded once chosen). No diabetic-retinopathy labels are used during pretraining; the objective is representation learning over fundus imagery (vascular topology, optic-disc and macular morphology, retinal texture, illumination variability, imaging artifacts). This represents an in-domain initialization with respect to the retinal-imaging domain while preserving the CNN architecture used in the baseline arm.
...
- **AOQ-1 — RESOLVED → option (b).** The V5 arm uses a CNN-compatible domain-adaptive self-supervised pretraining protocol (DINO / BYOL / SimCLR / MoCo family) on an unlabeled fundus corpus, not RETFound's ViT-Large. Architecture is **not** confounded with preprocessing: the same CNN backbones (ResNet-50, EfficientNet-B3) are used in both arms.
- **AOQ-2 — SIMPLIFIED.** Because the SSL pretraining is performed by the candidate, it may be run directly on 4-channel V5 tensors (RGB + FOV mask), eliminating any input-channel mismatch; alternatively the copy-and-mean-init protocol for channel 3 (`experiments/src/models/resnet.py` lines 47–52) remains available. No ViT patch-embedding adaptation is required. OD-3 Stage 3 (4-channel input) is preserved.
...
- **AOQ-4 — RESOLVED.** The 2×2 *(preprocessing × architecture)* factorial symmetry is restored: both ResNet-50 and EfficientNet-B3 run in both the baseline and the V5 arm (configs A/B/C/D in RESEARCH_ARCHITECTURE.md). The replication test across architecture families (EH-4) is reinstated.

The composite independent variable and CFC-2.8 are **retained**: the V5 arm still differs from baseline along both the preprocessing and the pretraining axis (ImageNet → ophthalmology-SSL), so the H-1 effect remains non-attributable to preprocessing alone.

---

**AOQ-1 [RESOLVED v6.0.0 → option (b)]: Backbone architecture in the V5 arm.**
...
*(Note: with in-house SSL pretraining the encoder can be trained directly on 4-channel V5 tensors, so the mismatch below — written for the externally-pretrained RETFound checkpoint — is largely moot. Retained for audit.)*
