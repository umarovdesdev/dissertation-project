...
> Draft generated per `prompts/writing-session-system-prompt.md` v6.0.0 · Brief: `briefs/3.3.4-brief.md` · Binding reference: INVARIANTS.md v6.0.0 · Sources: #21 (`yesmukhamedov-kbtu.md`, 🔹SELF, one thread with #19), #40 (`araf-2024.md`). FIG-3.12 referenced by placeholder, not embedded. Focal-loss canonical primary source (Lin et al. 2017) is a flagged corpus gap.
...
The training objective is the focal loss with an inverse-frequency class weighting (RESEARCH_ARCHITECTURE §4.0):
...
The loss is coordinated with the evaluation framework rather than standing alone. Because the DR grades are ordinal, a misclassification by two grades is clinically worse than a misclassification by one, and this ordinal cost is captured at evaluation time by the quadratic-weighted Cohen's κ that the evidence hierarchy places among the primary metrics (EH-1). The weighted loss thus addresses the imbalance during training while the ordinal-sensitive metric penalizes ordinal error during evaluation; both are specified in §3.4.1. One documentation limitation is recorded: the canonical primary source for the focal loss (Lin et al., 2017) is not present in the assembled literature corpus, a gap noted in §2.2.2 and carried here; the loss is specified per the dissertation's governance (RESEARCH_ARCHITECTURE §4.0) and its citation is flagged for corpus completion rather than supplied from an unverified reference [VERIFY: acquire focal-loss primary source, Lin et al. 2017]. With the adaptation, initialization, fine-tuning schedule, and loss now all specified, §3.4 turns to the framework by which the trained models are evaluated.
...
**Focal-loss source-gap honesty** — ✅ Satisfied. *"the canonical primary source for the focal loss (Lin et al., 2017) is not present in the assembled literature corpus … flagged for corpus completion rather than supplied from an unverified reference [VERIFY]"* (final ¶). No fabricated citation.
...
None for figures (FIG-3.12 `✅ AVAILABLE`). **Literature-acquisition flag:** focal-loss primary source (Lin et al. 2017) — corpus-completion queue (also flagged §2.2.2).
