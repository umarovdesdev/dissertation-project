...
**SIR-1 (non-amplification)** — ✅ Satisfied. #66/#68/#69/#70/#09 cited only for stated architectural facts.

**SIR-3 (metric consistency)** — ✅ Satisfied. #09 hybrid contextualized "on APTOS 2019 … no head-to-head comparison implied"; no cross-study metric transfer.
...
**DGL-6 (pretraining regimes; no guaranteed transfer)** — ✅ Satisfied. *"ImageNet for the baseline arm, ophthalmology-specific self-supervised pretraining for the V5 arm, with the degree of transfer treated as an empirical question rather than a guarantee (DGL-6)"* (final ¶); full treatment deferred to §3.3.

**DGL-2 (hardware-bound)** — ✅ Satisfied. *"a hardware- and implementation-bound setting … bounded to the documented hardware (DGL-2); it does not generalize to other compute contexts without re-evaluation"* (training-config ¶).
...
PART 1 body: ~1,015 words by manual count (within the 900–1,100 target; mid-band).
