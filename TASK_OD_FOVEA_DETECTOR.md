...
- [ ] Phase 1: trained weights + `eval_report.json` meet all acceptance criteria on IDRiD test.
- [ ] Phase 2: live pipeline uses the learned detector on the FOV-cropped frame; `validate_od_fovea_idrid.py` reproduces the numbers; INVARIANTS OD-3 updated + versions synced.
- [ ] Phase 3: demo draws OD/fovea discs + heatmap; user can drag-correct both centers and save.
- [ ] Phase 4: corrections persisted, exportable, and a documented fine-tune run regenerates frozen weights without regressing IDRiD-test acceptance.
